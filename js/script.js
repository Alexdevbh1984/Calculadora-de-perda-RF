function calcularPerdas() {
  ...
  const componentesSelecionados = Array.from(document.querySelectorAll('.componente:checked')).map(input => input.value);
  ...
  // Calcular perdas totais dos passivos
  let perdaTotalTX = 0;
  let perdaTotalRX = 0;
  ...
  componentesSelecionados.forEach(componente => {
    if (perdasPassivos[componente]) {
      perdaTotalTX += perdasPassivos[componente].tx; // Somar conforme o componente
      perdaTotalRX += perdasPassivos[componente].rx; // Subtrair conforme o componente
      detalhesComponentes.push(`- ${componente}: TX ${perdasPassivos[componente].tx} dB, RX ${perdasPassivos[componente].rx} dB`);
    }
  });

  // Perdas de cabo (ajustadas pela frequência e tipo de cabo)
  const perdaCabo = cabos * perdaCaboPorMetro;
  perdaTotalTX -= perdaCabo; // Cabo diminui o TX
